...
function fun_cut_set_instance_var(_id, _name, _value) {
	
	variable_instance_set(_id, _name, _value)
	
	fun_cut_end()
	
}

function fun_cut_set_instance_var_ext() { //supply each arg as an array [_id, name, _value]
	
	var i = 0
	repeat(argument_count) {
		
		variable_instance_set(argument[i][0], argument[i][1], argument[i][2])
...
function fun_cut_set_struct_var(_id, _name, _value) {
	
	variable_struct_set(_id, _name, _value)
	
	fun_cut_end()
	
}

function fun_cut_set_struct_var_ext() { //supply each arg as an array [_id, name, _value]
	
	var i = 0
	repeat(argument_count) {
		
		variable_struct_set(argument[i][0], argument[i][1], argument[i][2])
...
function fun_cut_set_global_var(_name, _value) {
...
function fun_cut_set_global_var_ext() { //supply each arg as an array [name, _value]
	
	var i = 0
	repeat(argument_count) {
		
		variable_global_set(argument[i][0], argument[i][1])
...
function fun_cut_flag_set(_index, _value, _relative = false) {
	
	fun_set_flag(_index, _value, _relative)
	
	fun_cut_end()
	
}

 #endregion change vars
